...
double* mpi_assemble_A (sed* A_loc, coupling_data* coupling){
    ...
    index n_loc = A_loc->n;
    index n_glob = coupling->n_globel_nodes;
    double* A_global = calloc(n_glob*n_glob, sizeof(double));
    gem* A_loc_full = sed_to_dense (A_loc, TRUE)
    incRow = A_loc_full->incRow;
    incCol = A_loc_full->incCol;
   
    //index* l2g = coupling->l2g;
    for (index i = 0; i<n_glob; i++){
        for (index j=0; j<n_glob; j++)
            A_global[l2g[i]*n_glob + l2g[j]] = A_loc_full[i*incRow + j*incCol];
    }
    if (rank == 0) {
