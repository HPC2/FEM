...
coupling_data* mpi_split_interfaces(interface_data* interfaces, index* l2g, int n_nodes, index n_global_nodes) {
    ...
    coupling->n_e_nodes = 0;
    for (int i = 0; i < coupling->ncoupl; i++) {
        coupling->n_e_nodes += coupling->dcoupl[i];
    }
    coupling->e_nodes = malloc(sizeof(index)*coupling->n_e_nodes);
    bool* buffer = calloc(n_nodes, sizeof(bool));
    int offset = 0;
    for (int i = 0; i < coupling->ncoupl; i++) {
        for (int j = 0; j < coupling->dcoupl[i]; j++) {
            coupling->e_nodes[offset++] = coupling->icoupl[i][j];
    ...
    coupling->n_i_nodes = n_nodes - coupling->n_e_nodes - coupling->n_local_cp;
